...
if (isset($_GET['id'])) {
    ...
    header("Location: ./admin/motorkerekpar-hirdetesek.php");
}
...
if (
    ...
    if (isset($_POST['upload'])) {
        ...
        if ($ok) {
            ...
            if (!empty($filesTempName)) {
                ...
                if ($_FILES["images"]["size"] > $maxSize) {
                    ...
                    if (is_uploaded_file($file) && !empty($file)) {
                        ...
                        $data_to_move = "../uploads/" . time() . $_FILES["images"]["name"];
                        ...
                        if ($_FILES["files"]["size"][$i] > $maxSize) {
                            ...
                            for ($i = 0; $i < $counted; $i++) {
                                ...
                                if (is_uploaded_file($file) && !empty($file)) {
                                    ...
                                    $data_to_move = "../uploads/" . time() . $_FILES["files"]["name"][$i];
                                    move_uploaded_file($file, $data_to_move);
                                    $is_mainimage = 0;
            ...
            echo "<script> window.location.replace('motorkerekpar-hirdetesek.php') </script>";
        }
        $link->close();
    }
    ...
    if (isset($_POST['delete'])) {
        ...
        if ($num_rows > 0) {
            ...
            echo copy($image_name, $deleted_image);
            unlink($image_name);
...
?>
        ...
        <div id="content">
            ...
            <div class="jumbotron">
                <form action="hirdetes-modositas.php?id=<?php echo htmlspecialchars($id); ?>" method="post" enctype='multipart/form-data' id="add">
                    ...
                    <ul class="fa-ul">
                        <li><span class="fa-li"><i class="fas fa-info-circle"></i></span><?= $moreImages > 0 ? 'További <b>' . htmlspecialchars($moreImages) . '</b> db képet tölthet fel.' : 'Elérte a maximálisan feltölthető képek számát.' ?></li>
                    </ul>
    ...
    header("Location: motorkerekpar-hirdetesek.php");
}
    ?>
